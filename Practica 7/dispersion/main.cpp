...
{
    unsigned int i;
    for( i = 0; i < sizeof( id ); id[ i++ ] = '\0' );
    for( i = 0; i < sizeof( nombre ); nombre[ i++ ] = '\0' );
    for( i = 0; i < sizeof( membresia ); membresia[ i++ ] = '\0' );
    for( i = 0; i < sizeof( precio ); precio[ i++ ] = '\0' );
    for( i = 0; i < sizeof( fecha ); fecha[ i++ ] = '\0' );
}
...
void Pago::genera( void )
{
    Pago pago;
    int contador = 0;
    fstream hayArchivo("dispersion2.txt");
    if(!hayArchivo)
    {
...
int Pago::dispersion( const char llave[ 12 ] )
{
    ...
    return ( sum % 99 ); // retorna la posición donde se guardará el registro.
}
...
void Pago::setMembresia( const string &valorMembresia )
{
...
ostream &operator<<( ostream &salida, const Pago &pago )
{
    salida << "ID:        " << pago.id << endl
            << "Nombre:    " << pago.nombre << endl
            << "Membresia: " << pago.membresia << endl
...
int main( void )
{
    ...
    do{
        ...
        cout<<"\t    Dispersion con Manejo de Colisiones\n\n";
